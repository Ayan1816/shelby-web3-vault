...
function ShelbyVault() {
  ...
  useEffect(() => {
    setMounted(true);
    const savedHistory = localStorage.getItem("shelby_vault_history_v3");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response && response.hash) {
        ...
        localStorage.setItem("shelby_vault_history_v3", JSON.stringify(newHistory));
    ...
    <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center p-4 font-sans selection:bg-fuchsia-500/30 pb-20 relative">
          ...
          {connected && account ? (
            <div className="flex flex-wrap items-center gap-3">
              <button 
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-fuchsia-500/10 hover:bg-fuchsia-500/20 border border-fuchsia-500/30 text-fuchsia-400 font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50"
              >
                {isFauceting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
      ...
      <div className="w-full max-w-6xl mt-4 flex flex-wrap justify-between items-center bg-white/[0.02] border border-white/5 rounded-lg px-6 py-3 text-xs font-mono text-gray-400">
        <div className="flex items-center gap-2">
          ...
          <span>SECURED NODE: {network?.name || 'OFFLINE'}</span>
        </div>
          ...
          <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 h-full min-h-[400px]">
            <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
              <div className="flex items-center gap-2 text-gray-300">
                ...
                <h3 className="font-bold tracking-wider text-sm uppercase">My Secure Vault</h3>
              </div>
                  ...
                  <div key={index} className="w-full bg-black/40 border border-white/10 rounded-lg p-3 hover:border-cyan-500/50 transition-all">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-400 font-mono">Secured Asset</span>
                      </div>
                      <span className="text-[10px] text-gray-500 font-mono">{new Date(record.timestamp).toLocaleString()}</span>
                    </div>
                    
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-gray-300 truncate w-32">
                          {record.type === 'file' ? record.fileName : 'Secret Text Payload'}
                        </span>
                        <a 
                          href={`https://explorer.aptoslabs.com/txn/${record.hash}?network=${network?.name?.toLowerCase() || 'testnet'}`}
                          target="_blank" rel="noreferrer"
                          className="text-[10px] font-mono text-cyan-400 hover:underline truncate w-32 mt-1"
                        >
                          Verify on Explorer
                        </a>
                      </div>
                      ...
                      <button 
                        onClick={() => handleUnlock(record.hash)}
                        className="bg-fuchsia-600/20 hover:bg-fuchsia-600/40 text-fuchsia-400 border border-fuchsia-500/30 px-3 py-1.5 rounded-md text-[10px] font-bold flex items-center gap-1 transition-colors"
                      >
