...
<button onClick={() => setShowNotif(!showNotif)} className={`relative p-2 rounded-xl border ${light ? 'bg-slate-100 border-slate-300 text-slate-600 hover:bg-slate-200' : 'bg-white/5 border-white/10 text-gray-400 hover:text-white'}`}><Bell className="w-5 h-5"/><span className="absolute top-0 right-0 w-2.5 h-2.5 bg-pink-500 rounded-full border border-current"></span></button>
{showNotif && <div className={`absolute top-full right-0 mt-2 w-48 rounded-xl shadow-2xl border p-3 z-50 animate-in fade-in zoom-in-95 ${light ? 'bg-white border-slate-200' : 'bg-[#0f0f0f] border-white/10'}`}><p className={`text-xs text-center font-bold ${light ? 'text-slate-500' : 'text-gray-400'}`}>No new notifications</p></div>}
</div>
<button onClick={() => setLight(!light)} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm border ${light ? 'bg-slate-100 border-slate-300 text-slate-800 hover:bg-slate-200' : 'bg-white/5 border-white/10 text-yellow-400 hover:bg-white/10'}`}>{light ? <><Moon className="w-4 h-4"/> Dark</> : <><Sun className="w-4 h-4"/> Light</>}</button>
{conn && acc ? (
<><button onClick={() => doFaucet('apt')} className="px-3 py-2 rounded-lg font-bold text-xs uppercase bg-blue-500/10 border border-blue-500/30 text-blue-500 hover:bg-blue-500/20"><Zap className="w-3.5 h-3.5 inline mr-1"/> APT Faucet</button><button onClick={() => doFaucet('shelby')} className="px-3 py-2 rounded-lg font-bold text-xs uppercase bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-500 hover:bg-fuchsia-500/20"><Zap className="w-3.5 h-3.5 inline mr-1"/> S-USD Faucet</button><div className="px-4 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 font-mono"><Coins className="w-4 h-4 inline mr-1"/><span className="text-sm font-bold">{bal} APT</span></div><button onClick={copyAddr} className={`flex items-center gap-2 border px-4 py-2 rounded-lg ${light ? 'bg-slate-50 border-slate-200' : 'bg-white/5 border-white/10'}`}><span className="text-sm font-mono text-fuchsia-500">{acc.address?.slice(0,6)}...{acc.address?.slice(-4)}</span>{copied ? <CheckCircle2 className="w-4 h-4 text-green-500"/> : <Copy className="w-4 h-4 text-gray-400"/>}</button><button onClick={disconn} className="p-2.5 bg-red-500/10 border border-red-500/30 rounded-lg text-red-500 hover:bg-red-500/20"><LogOut className="w-4 h-4"/></button></>
) : <button onClick={() => wlts?.length ? cnct(wlts[0].name) : alert("Install Petra!")} className="flex items-center gap-2 bg-gradient-to-r from-fuchsia-600 to-purple-600 px-8 py-3 rounded-xl font-bold text-white hover:from-fuchsia-500 hover:to-purple-500"><Wallet className="w-5 h-5"/> Connect Wallet</button>}
</div>
</header>
...
<div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${conn ? 'bg-green-500 animate-pulse shadow-[0_0_10px_#22c55e]' : 'bg-red-500 shadow-[0_0_10px_#ef4444]'}`}></div><span className={conn ? 'text-green-500 font-bold' : 'text-red-500 font-bold'}>{conn ? `NODE: ${net?.name?.toUpperCase() || 'TESTNET'}` : 'OFFLINE'}</span></div>
...
<button onClick={() => setTab('vault')} className={`px-6 py-3 rounded-xl font-bold text-xs flex items-center gap-2 border transition-all ${tab === 'vault' ? 'bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white border-transparent shadow-lg shadow-cyan-500/20 scale-105' : (light ? 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' : 'bg-white/[0.02] border-white/5 text-gray-400 hover:text-white')}`}><Shield className="w-4 h-4"/> 1. Vault Workshop</button>
<button onClick={() => setTab('scanner')} className={`px-6 py-3 rounded-xl font-bold text-xs flex items-center gap-2 border transition-all ${tab === 'scanner' ? 'bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white border-transparent shadow-lg shadow-cyan-500/20 scale-105' : (light ? 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' : 'bg-white/[0.02] border-white/5 text-gray-400 hover:text-white')}`}><Search className="w-4 h-4"/> 2. Integrity Scanner</button>
<button onClick={() => setTab('gallery')} className={`px-6 py-3 rounded-xl font-bold text-xs flex items-center gap-2 border transition-all ${tab === 'gallery' ? 'bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white border-transparent shadow-lg shadow-cyan-500/20 scale-105' : (light ? 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' : 'bg-white/[0.02] border-white/5 text-gray-400 hover:text-white')}`}><FolderGit2 className="w-4 h-4"/> 3. Decrypted Studio {gal.length > 0 && <span className="px-2 py-0.5 rounded-full bg-fuchsia-500 text-white font-black text-[10px] ml-1">{gal.length}</span>}</button>
</nav>
...
<div className={`border rounded-xl p-6 relative ${light ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0a0a0a] border-white/10'}`}>
<div className={`flex gap-4 mb-6 border-b pb-4 ${light ? 'border-slate-100' : 'border-white/5'}`}><button onClick={() => setVMode('text')} className={`text-sm font-bold pb-2 ${vMode === 'text' ? 'text-fuchsia-500 border-b-2 border-fuchsia-500' : 'text-gray-400 hover:text-gray-500'}`}><FileText className="w-4 h-4 inline mr-1"/> Secret Text</button><button onClick={() => setVMode('file')} className={`text-sm font-bold pb-2 ${vMode === 'file' ? 'text-fuchsia-500 border-b-2 border-fuchsia-500' : 'text-gray-400 hover:text-gray-500'}`}><UploadCloud className="w-4 h-4 inline mr-1"/> IPFS File Vault</button></div>
{vMode === 'text' ? <textarea value={code} onChange={(e) => setCode(e.target.value)} placeholder="Type highly sensitive data here..." className={`w-full h-40 border rounded-lg p-4 text-sm font-mono outline-none resize-none ${light ? 'bg-slate-50 border-slate-200 text-slate-800 focus:border-fuchsia-400' : 'bg-black/60 border-white/5 text-gray-300 focus:border-fuchsia-500/50'}`} /> : <div className={`w-full h-40 border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer ${drag ? 'border-fuchsia-500 bg-fuchsia-500/10' : (light ? 'border-slate-300 bg-slate-50 hover:border-slate-400' : 'border-white/10 bg-black/40 hover:border-white/30')}`} onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={(e) => { e.preventDefault(); setDrag(false); if(e.dataTransfer.files[0]) setSFile(e.dataTransfer.files[0]); }} onClick={() => fileRef.current?.click()}><input type="file" ref={fileRef} className="hidden" onChange={(e) => e.target.files?.[0] && setSFile(e.target.files[0])} />{sFile ? <div className="text-center"><FileIcon className="w-8 h-8 text-fuchsia-500 mx-auto mb-2"/><span className="text-sm font-bold text-fuchsia-500">{sFile.name}</span></div> : <div className="text-center text-gray-500"><UploadCloud className="w-8 h-8 mx-auto mb-2"/><span className="text-sm font-bold">Select File (Max 10MB)</span></div>}</div>}
...
<div className={`border rounded-2xl p-5 h-[450px] flex flex-col ${light ? 'bg-white border-slate-200 shadow-sm' : 'bg-white/[0.02] border-white/10'}`}>
<h3 className={`font-bold text-sm uppercase flex items-center gap-2 mb-4 border-b pb-4 ${light ? 'border-slate-100' : 'border-white/5'}`}><Globe className="w-4 h-4 text-cyan-500"/> Live Blockchain History</h3>
<div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
{!conn ? <div className="text-center text-gray-500 py-10">Connect wallet to view history.</div> : cHist.length === 0 ? <div className="text-center text-gray-500 py-10">No transactions.</div> : cHist.map((tx, i) => { const loc = hist.find(h => h.hash === tx.hash); return <div key={i} className={`border rounded-lg p-3 ${light ? 'bg-slate-50 border-slate-200 hover:border-cyan-400' : 'bg-black/40 border-white/10 hover:border-cyan-500/50'}`}><div className="flex justify-between mb-2"><span className="text-[10px] text-gray-400"><Activity className="w-3 h-3 inline text-cyan-500"/> Ver: {tx.version}</span><span className="text-[10px] text-gray-500">{new Date(tx.timestamp).toLocaleString()}</span></div><div className="flex justify-between items-center"><a href={`https://explorer.aptoslabs.com/txn/${tx.hash}?network=${net?.name?.toLowerCase() || 'testnet'}`} target="_blank" rel="noreferrer" className="text-xs font-mono text-cyan-500 hover:underline truncate w-24">{tx.hash.slice(0,12)}...</a>{loc ? <div className="flex gap-1.5"><button onClick={() => openShare(loc)} className="bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 px-2.5 py-1.5 rounded-md text-[10px] font-bold"><QrCode className="w-3 h-3 inline"/> Share</button><button onClick={() => setSHash(tx.hash)} className="bg-fuchsia-500/10 text-fuchsia-500 hover:bg-fuchsia-500/20 px-3 py-1.5 rounded-md text-[10px] 
