...
import { Copy, CheckCircle2, Shield, LogOut, Wallet, Database, Coins, Key, Lock, Unlock, X, FileText, UploadCloud, File as FileIcon, Globe, Zap } from "lucide-react";
...
function ShelbyVault() {
  ...
  useEffect(() => {
    setMounted(true);
    const savedHistory = localStorage.getItem("shelby_vault_v6");
  ...
  const fetchBalanceAndTx = async () => {
    ...
    // 🚀 Dual-Engine Balance Fetcher (Bypasses Mobile Blocks)
    try {
      const net = network?.name?.toLowerCase().includes('mainnet') ? 'mainnet' : 'testnet';
      const url1 = `https://fullnode.${net}.aptoslabs.com/v1/accounts/${account.address}/resource/0x1::coin::CoinStore%3C0x1::aptos_coin::AptosCoin%3E`;
      const url2 = `https://api.${net}.aptoslabs.com/v1/accounts/${account.address}/resource/0x1::coin::CoinStore%3C0x1::aptos_coin::AptosCoin%3E`;

      let res = await fetch(url1).catch(() => null);
      if (!res || !res.ok) res = await fetch(url2).catch(() => null);
      ...
      // Fetch History
      const txRes = await fetch(`https://fullnode.${net}.aptoslabs.com/v1/accounts/${account.address}/transactions?limit=20`);
      if (txRes.ok) {
  ...
  useEffect(() => {
    if (connected) {
      fetchBalanceAndTx();
      const interval = setInterval(fetchBalanceAndTx, 10000);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response && response.hash) {
        ...
        setHistory(newHistory); localStorage.setItem("shelby_vault_v6", JSON.stringify(newHistory));
    ...
    <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center p-4 font-sans selection:bg-fuchsia-500/30 pb-20">
      <header className="w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 py-5 px-6 mt-4 bg-white/[0.02] border border-white/5 rounded-2xl shadow-2xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {connected && account ? (
            <>
              <a href="https://aptoslabs.com/testnet-faucet" target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-400 font-bold text-xs uppercase hover:bg-fuchsia-500/20"><Zap className="w-3.5 h-3.5" /> Faucet</a>
      ...
      <div className="w-full max-w-6xl flex flex-col lg:flex-row gap-6 mt-6">
        ...
        <aside className="w-full lg:w-96 flex flex-col gap-4">
          <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 h-[450px] flex flex-col">
            <h3 className="font-bold text-sm uppercase flex items-center gap-2 mb-4 border-b border-white/5 pb-4"><Globe className="w-4 h-4 text-cyan-400" /> My Secure Vault</h3>
