...
function ShelbyVault() {
  ...
  // 🚀 রিয়েল-টাইম নোটিফিকেশন পুশ করার মাস্টার ফাংশন
  ...
  // 🚀 ইভেন্ট লিসেনার ১: কানেকশন সাকসেস হলে নোটিফিকেশন পাঠাবে
  useEffect(() => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response && response.hash) {
        ...
        // 🚀 ইভেন্ট লিসেনার ২: ট্রানজাকশন সাকসেস
      ...
      // 🚀 ইভেন্ট লিসেনার ৩: ট্রানজাকশন কেটে দেওয়া বা ফেইল হওয়া
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center p-4 font-sans pb-20 transition-colors duration-500 relative ${isLightMode ? 'bg-[#f8f9fa] text-slate-900' : 'bg-[#050505] text-white'}`}>
      
      {/* 🔴 আপনার চাওয়া মতো একদম উপরের ডানদিকের কোণায় আলাদা আইসোলেটেড থিম ও নোটিফিকেশন বার */}
      <div className="w-full max-w-6xl flex justify-end items-center gap-3 pt-2 px-2">
        
        {/* নোটিফিকেশন ড্রপডাউন সিস্টেম */}
        <div className="relative">
          <button 
            onClick={() => { setShowNotifPanel(!showNotifPanel); setUnreadCount(0); }} 
            className={`relative flex items-center gap-2 px-3.5 py-2 rounded-xl border text-xs font-bold transition-all shadow-sm ${isLightMode ? 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' : 'bg-white/10 border-white/15 text-gray-200 hover:bg-white/20'}`}
          >
            ...
            <div className={`absolute top-full right-0 mt-2 w-80 sm:w-96 rounded-2xl shadow-2xl border p-4 z-50 animate-in fade-in zoom-in-95 space-y-3 ${isLightMode ? 'bg-white border-slate-200 text-slate-800' : 'bg-[#121212] border-white/20 text-white'}`}>
              ...
              <div className="max-h-60 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                {notifications.length === 0 ? (
                  ...
                  notifications.map(n => (
                    <div key={n.id} className={`p-2.5 rounded-xl border text-xs flex items-start gap-2.5 ${n.type === 'success' ? (isLightMode ? 'bg-emerald-50 border-emerald-200' : 'bg-emerald-950/30 border-emerald-500/30 text-emerald-300') : (n.type === 'error' ? (isLightMode ? 'bg-rose-50 border-rose-200' : 'bg-rose-950/30 border-rose-500/30 text-rose-300') : (isLightMode ? 'bg-slate-50 border-slate-200' : 'bg-white/5 border-white/10 text-cyan-300'))}`}>
        ...
        <button 
          ...
          className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-bold text-xs border transition-all shadow-sm ${isLightMode ? 'bg-slate-800 border-slate-700 text-white hover:bg-slate-900' : 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/20'}`}
        >
          {isLightMode ? <><Moon className="w-3.5 h-3.5"/> Dark</> : <><Sun className="w-3.5 h-3.5"/> Light</>}
        </button>
      </div>

      {/* 🔴 মূল ওয়ালেট হেডার (উপরে ডানদিকের বার থেকে সম্পূর্ণ আলাদা) */}
      <header className={`w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-4 py-5 px-6 mt-3 rounded-2xl shadow-lg border transition-colors ${isLightMode ? 'bg-white border-slate-200' : 'bg-white/[0.02] border-white/5'}`}>
